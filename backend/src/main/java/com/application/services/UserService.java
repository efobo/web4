...
public class UserService implements UserDetailsService {
    ...
    public ResponseEntity<String> login(UserDTO userDTO){
        ...
        try {
            ...
            log.error("Пользователь попытался авторизоваться, но ввел не те данные");
            return new ResponseEntity<>("Неверные учетные данные пользователя", HttpStatus.BAD_REQUEST);
        } catch (Exception e) {
            log.error("Произошла беда, при попытке авторизоваться", e.getMessage());
    ...
    public ResponseEntity<String> register(UserDTO userDTO){
        try {
            ...
            log.error("Произошла беда, при попытке авторизоваться", e.getMessage());
            return new ResponseEntity<>("Произошла беда, при попытке авторизоваться", HttpStatus.BAD_REQUEST);
        }
    }
