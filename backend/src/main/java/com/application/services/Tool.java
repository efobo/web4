...
public class Tool {
    ...
    public String checkValid(Map<String, String> request,double x,double y,double r){
        ...
        if (x>-3&&x<=5){
            ...
            stringForReturn="x выходит за рамки дозволенного значения X ∈ (-3;5)";
        }
        if(y>-5&&y<3){

        }else {

            stringForReturn="Y выходит за рамки дозволенного значения Y ∈ (-5;3)";
        }
        if(r>0&&r<=5){

        }else {

            stringForReturn="R выходит за рамки дозволенного значения R ∈ (0;5]";
        }
