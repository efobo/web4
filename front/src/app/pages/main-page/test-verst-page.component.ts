...
export class TestVerstPageComponent implements OnInit {
  ...
  constructor(private _test: AuthGuard, private _dropAll: DeleteAllPointsService,
              private _tokenService: TokenService, private _getPointService: GetPointsService,
              private _checkPointService: CheckPointService, private _router: Router) {
   this.pointY = 0;
  }
  ...
  private checkPoint(xInput: number, yInput: number, rInput: number): void {
    ...
    this._checkPointService.checkPoints(sendData).subscribe((res: any) => res,
      (err: any) => {if (String(err.body) === '[object ProgressEvent]'){
        alert('Сервер не отвечает, повторите запрос чуть позже');
      } else {
        if (String(err.body) === 'unAUT'){
  ...
  public clickOnSVG(event: any){
    ...
    if (xi > -3 && xi < 5){
      ...
      this.errorText = 'X выходит за ОДЗ';
    }
